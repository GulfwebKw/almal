...
class AdminCorporateOverviewController extends Controller
{
    public $settings;
    public $model;
    public $path;
    ...
    public function __construct()
    {
        ...
        $this->model = '\App\CorporateOverview';
        $this->title = 'Corporate Overview';
        $this->path = 'corporateoverview';
    ...
    public function store(Request $request)
    {
        ...
        if (request()->hasFile('image_top')) {
            $image_top = Common::uploadImage($request, 'image_top', $this->path, $this->image_big_w, $this->image_big_h, $this->image_thumb_w, $this->image_thumb_h);
        }
        $resource = new CorporateOverview();
        $resource->name = $request->input('name');
        $resource->title_en = $request->input('title_en');
        $resource->title_ar = $request->input('title_ar');
        $resource->short_description_en = $request->input('short_details_en');
        $resource->short_description_ar = $request->input('short_details_ar');
        $resource->description_en = $request->input('details_en');
        $resource->description_ar = $request->input('details_ar');
...
//        ]);
        ...
        $resource = CorporateOverview::find($id);

        $image = $resource->image;
        $image_top = $resource->image_top;
        if ($request->hasfile('image')) {
        ...
        if (request()->hasFile('image_top')) {
            $image_top = Common::editImage($request, 'image_top', $this->path, $this->image_big_w, $this->image_big_h, $this->image_thumb_w, $this->image_thumb_h, $resource);
        }
        $resource->name = $request->input('name');
        $resource->title_en = $request->input('title_en');
        $resource->title_ar = $request->input('title_ar');
    ...
    public function destroy($id)
    {
        ...
        if ($imageFieldName2) {
            if (!empty($resource->$imageFieldName2)) {
                Common::deleteImage($imageFieldName2, $this->path, $resource);
            }
        }
