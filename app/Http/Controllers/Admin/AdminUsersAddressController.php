...
class AdminUsersAddressController extends Controller
{
    ...
    {
        $resources = $this->model::find($id)->address()->paginate($this->settings->item_per_page_back);
    ...
    public function store(Request $request, $id)
    {
        ...
        $resource->zipcode = $request->input('zipcode');
    ...
    public function update(Request $request, $user_id, $id)
    {
        ...
        $resource->full_name = $request->input('full_name');
        $resource->mobile = $request->input('mobile');
        $resource->country = Country::find($request->country_id)->title_en;
    ...
    public function destroy($user_id, $address_id)
    {
        ...
        $resource = $this->model::find($user_id)->address;
        if (count($resource) > 0) {
            foreach ($resource as $item) {
                if ($item->id == $address_id) {
                    ...
                    $key_name = $this->title;
                    $key_id = $item->id;
                    $message = "A record is removed. (" . $item->id . ")";
                    $created_by = Auth::guard('admin')->user()->id;
                    Common::saveLogs($key_name, $key_id, $message, $created_by);
                }
            }
