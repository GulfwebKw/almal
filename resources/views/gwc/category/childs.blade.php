<div class="kt-list-timeline__items">
    ...
    @foreach($childs as $child)
         ...
         @endfor
         <span class="kt-list-timeline__text" style="color: cornflowerblue">{{$child->name_en}}</span>
         
         <span class="kt-list-timeline__time">
         @if($child->image)
         <img src="{!! url('uploads/category/'.$child->image) !!}" width="35">
         @endif
         </span>
         <span class="kt-list-timeline__time">
         ...
         <!--action-->
          <span style="overflow: visible; position: relative; width: 80px;">
                                                 ...
                                                 <li class="kt-nav__item"><a href="{{url('gwc/category/'.$child->id.'/edit')}}" class="kt-nav__link"><i class="kt-nav__link-icon flaticon2-contract"></i><span class="kt-nav__link-text">edit</span></a></li>
                                                 @endif
                                                
                                                 @if(auth()->guard('admin')->user()->can('category-delete'))
                                                 <li class="kt-nav__item"><a href="javascript:;" data-toggle="modal" data-target="#kt_modal_{{$child->id}}" class="kt-nav__link"><i class="kt-nav__link-icon flaticon2-trash"></i><span class="kt-nav__link-text">delete</span></a></li>
                                                 @endif
                                                 </ul>
                                                 </div>
                                                 </div>
								...
								<div class="modal-dialog" role="document">
									<div class="modal-content">
										...
										<div class="modal-body">
											<h6 class="modal-title text-left">Do you want delete this child</h6>
										</div>
										<div class="modal-footer">
											<button type="button" class="btn btn-secondary" data-dismiss="modal">no</button>
											<button type="button" class="btn btn-danger"  onClick="Javascript:window.location.href='{{url('gwc/category/delete/'.$child->id)}}'">yes</button>
										</div>
									</div>
								</div>
							</div>
